...
let ``Can reroot a tree with new root deeply nested in tree``() =
    let tree =
        mkGraph "level-0" [ mkGraph "level-1" [ mkGraph "level-2" [ mkGraph "level-3" [ mkGraph "x" [] ] ] ] ]
    let expected =
        mkGraph "x" [ mkGraph "level-3" [ mkGraph "level-2" [ mkGraph "level-1" [ mkGraph "level-0" [] ] ] ] ]
    fromPOV "x" tree
    |> mapToList
    |> should equal
    <| graphToList expected
...
let ``Can find path to parent``() =
    let tree =
        mkGraph "parent"
            [ mkGraph "x" []
              mkGraph "sibling" [] ]
    tracePathBetween "x" "parent" tree
    |> should equal
    <| Some [ "x"; "parent" ]
        ...
        mkGraph "grandparent"
            [ mkGraph "parent"
              ...
              mkGraph "uncle"
                  [ mkGraph "cousin-0" []
                    mkGraph "cousin-1" [] ] ]
    tracePathBetween "x" "cousin-1" tree
    |> should equal
    <| Some [ "x"; "parent"; "grandparent"; "uncle"; "cousin-1" ]
        ...
        mkGraph "grandparent"
            [ mkGraph "parent"
                  [ mkGraph "x" []
                    mkGraph "sibling-0" []
                    mkGraph "sibling-1" [] ] ]
    tracePathBetween "x" "sibling-1" tree
    |> should equal
    <| Some [ "x"; "parent"; "sibling-1" ]
